...
public class ViewResult extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i <= valueArray.length - 1; i++) {
            ...
            String valueText;
            if(valueArray[i] == Constants.SWITCH_OFF || valueArray[i] == Constants.SWITCH_DISABLE_OFF){
                valueText = text1;
            }else if(valueArray[i] == Constants.SWITCH_ON || valueArray[i] == Constants.SWITCH_DISABLE_ON){
                valueText = text2;
            }else if(valueArray[i] == Constants.SWITCH_LATER){
                valueText = text3;
            }else{
                valueText = textElse;
            }
